...
export async function GET() {
  try {
    ...
    try {
      ...
      console.log(
        `Output file already exists at ${outputPath}. Overwriting...`
      );
      ...
      if (
        ...
        console.log(
          `No existing file at ${outputPath}. Proceeding with generation.`
        );
      } else {
      ...
      try {
        ...
        if (stats.size > 0) {
          ...
          if (duration > 0 && duration < 600) {
            // Ignore files longer than 10 minutes
            fileList.push(`file '${filePath.replace(/'/g, "'\\''")}'`);
          } else {
            console.warn(
              `Ignoring file ${file} due to invalid duration: ${duration}s`
            );
          }
        }
      } catch (error) {
        if (error instanceof Error) {
          ...
          console.error(`Unknown error processing file ${filePath}:`, error);
        }
      }
    }
