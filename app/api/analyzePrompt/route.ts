...
async function extractParameters(prompt) {
    try {
      const response = await axios.post('https://api.openai.com/v1/chat/completions', {
        model: 'gpt-4',
        messages: [
          { role: 'system', content: 'You are an assistant that extracts video parameters from a user prompt.' },
          {
            role: 'user',
...
export async function POST(req) {
  try {
    ...
    const { topic, style, language, duration, ssmlGender, voiceName } = await extractParameters(prompt);
    console.log('Extracted parameters:', { topic, style, language, duration, ssmlGender, voiceName });

    // Step 2: Call the existing video generation API
    let videoResponse;
    try {
      videoResponse = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/generateVideo`, {
        topic,
        style,
        language,
        duration,
    ...
    return NextResponse.json({ error: 'Unexpected error in processing prompt', details: error.message }, { status: 500 });
  }
}
